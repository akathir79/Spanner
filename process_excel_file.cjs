  ...
  try {
    ...
    workbook.SheetNames.forEach((sheetName, index) => {
      ...
      if (jsonData.length > 0) {
        ...
        // Write data to JSON file for manual processing
        fs.writeFileSync(`excel_data_${sheetName.replace(/[^a-zA-Z0-9]/g, '_')}.json`, 
          JSON.stringify(jsonData, null, 2));
        console.log(`Data exported to excel_data_${sheetName.replace(/[^a-zA-Z0-9]/g, '_')}.json`);
      }
    });
